//! Keeping a GFM table aligned while it is being edited.
//!
//! Neovim owns the two things only it can know: which buffer line the cursor
//! is on, and how to write the result back without disturbing marks and undo
//! history. Everything else happens here, and none of it is a Markdown parser:
...
/// Format the table under `(row, col)`, if there is one.
///
...
pub fn format(input: &str, settings: &Settings, row: usize, col: usize) -> Option<TableEdit> {
    ...
    let table = find_table(parse_document(&arena, input, &options), row)?;

    let alignments = match &table.data().value {
        NodeValue::Table(node) => node.alignments.clone(),
        _ => return None,
    };
    let span = table.data().sourcepos;
    let (first, last) = (span.start.line, span.end.line);

    let rows = collect_rows(table, &lines)?;
    // The delimiter row is not a node of its own: comrak folds it into the
    // table's alignments. It is always the line after the header.
    let separator = rows.first()?.line + 1;
    if separator > last {
        return None;
    }

    // The header's prefix is used for every line of the table, so a row that
    // was quoted or indented more loosely than its header comes back matching
    // it.
    let prefix = rows.first()?.prefix;
    let widths = widths(&rows, &alignments);

    let mut replacement = Vec::with_capacity(last - first + 1);
    for line in first..=last {
        if line == separator {
            replacement.push(render(prefix, &delimiters(&alignments, &widths)));
        } else if let Some(row) = rows.iter().find(|row| row.line == line) {
            replacement.push(render(prefix, &padded(row, &widths)));
        } else {
            // Unreachable while comrak gives every table line a row, and
            // harmless if that ever changes: the line is left as it was.
            replacement.push(lines[line - 1].to_string());
        }
    }

    let cursor_col = cursor(&rows, &widths, prefix.len(), row, col)
        .unwrap_or_else(|| col.min(replacement[row - first].len()));
...
/// The innermost table covering `row`. Tables cannot nest inside one another,
/// so at most one node matches.
fn find_table<'a>(root: &'a AstNode<'a>, row: usize) -> Option<&'a AstNode<'a>> {
    root.descendants().find(|node| {
        let data = node.data();
        matches!(data.value, NodeValue::Table(_))
            && data.sourcepos.start.line <= row
            && row <= data.sourcepos.end.line
    })
}
...
/// Where the cursor lands once the table has been rewritten: the same offset
/// into the same cell it was in before, which is what keeps typing in a cell
/// from being interrupted by the realignment around it.
///
/// `None` when the cursor is not on a row, which means it is on the delimiter
/// line.
fn cursor(rows: &[Row], widths: &[usize], prefix: usize, row: usize, col: usize) -> Option<usize> {
    let cells = &rows.iter().find(|candidate| candidate.line == row)?.cells;
    ...
    // Two for the `| ` the line opens with, three for each ` | ` crossed on
    // the way to this column.
    let start = prefix
        + 2
        + widths[..column.min(widths.len())]
            .iter()
            .map(|w| w + 3)
            .sum::<usize>();
    Some(start + offset)
